...
class faulttolerance {
	...
	def gennode1 = {
		...
		NodeNameList.each {
			...
			if (!isServerAvail()) {
				// JV: Is this logic correct? False means server is up, not down
				// GD: This logic will be modified for isServerAvail OR isRunningServiceLess.Right now ! is for testing
	...
	public void safestart() {
		ScriptExecutionCauseException ex = scriptShouldFail { start() }
	}

	public void safeloadbalance() {
		ScriptExecutionCauseException vx = scriptShouldFail { loadbalance() }
	}
	...
	def Integer isServerup() {
		...
		// JV: Is stale list = down server list? G
		...
		//GD : glulcheck.sh check wheather glu agent is running or not.? or check if server has gone down ?
